...
// Available body-content height per page.
// Actual measured overheads (see inline CSS on each chrome element):
//   Page 1:  full header ~76px + severity bar ~44px + footer ~37px + body padding 44px = 201px → avail 922px
//   Page 2+: compact header ~42px + cont-strip ~27px + footer ~37px + body padding 44px = 150px → avail 973px
// We subtract a 15–20 px safety buffer to guard against font-metric rounding.
...
const TEAM_SPLIT_HINT = 30;
const SEC_TITLE_EST = 31;
const TEAM_FIRST_OVERHEAD =
  SEC_TITLE_EST + TEAM_SPLIT_BOX_HEADER + TEAM_SPLIT_HINT;
const TEAM_CONT_OVERHEAD = SEC_TITLE_EST;

// Ordered-list (timeline) split constants
const TL_ITEM_GAP = 7; // gap between TimelineRow items
const FIELD_GAP = 10; // gap between field runs in GenericFieldList
const TL_SPLIT_HINT_H = 24; // "continued" hint row at bottom of first slice
...
function TimelineRow({
  ...
  item: Extract<ReportField, { type: "ordered-list" }>["items"][number];
}) {
  return (
...
function FieldTimeline({
  ...
  field: Extract<ReportField, { type: "ordered-list" }>;
}) {
...
// Single team member row — border between rows is applied by the parent list, not here
function TeamMemberRow({
  member,
}: {
  member: Extract<ReportField, { type: "team-table" }>["members"][number];
}) {
  return (
    <div
      style={{
    ...
    >
      ...
      >
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <circle cx="12" cy="8" r="4" fill={C.greenMid} />
          <path d="M4 20c0-4 3.582-7 8-7s8 3 8 7" stroke={C.greenMid} strokeWidth="2" strokeLinecap="round" fill="none" />
        </svg>
      </div>
      <div>
        <div
...
function FieldTeamList({
  field,
}: {
  field: Extract<ReportField, { type: "team-table" }>;
}) {
    ...
    >
      {field.members.map((m, idx) => (
        <div
          key={m.id}
          style={
            idx > 0 ? { borderTop: `0.5px solid ${C.cardBorder}` } : undefined
          }
        >
...
function InfoCardGrid({
  ...
  fields: Array<
    Extract<ReportField, { type: "text-input" | "select" | "date-time" }>
  >;
}) {
  const cols =
    fields.length === 1
  ...
  for (const f of fields) {
    if (
      ...
      grid.push(
        f as Extract<
          ReportField,
          { type: "text-input" | "select" | "date-time" }
        >,
      );
    } else {
      if (grid.length) {
        runs.push({ kind: "info-grid", fields: grid });
        grid = [];
      }
...
function GenericFieldList({ fields }: { fields: ReportField[] }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {runs.map((run, i) => {
        if (run.kind === "info-grid")
          return <InfoCardGrid key={i} fields={run.fields} />;
        const f = run.field;
        if (f.type === "rich-text")
          return <FieldRichText key={f.id} html={f.value} />;
        if (f.type === "ordered-list")
          return <FieldTimeline key={f.id} field={f} />;
        if (f.type === "team-table")
          return <FieldTeamList key={f.id} field={f} />;
        return null;
      })}
    </div>
  );
...
function IncidentClosureSection({ group }: { group: ReportGroup }) {
  const richField = group.fields.find((f) => f.type === "rich-text") as
    | Extract<ReportField, { type: "rich-text" }>
    | undefined;
  const msg =
    richField?.value
      ...
      .replace(/<[^>]+>/g, "")
      .replace(/\s+/g, " ")
      .trim() || "Incident closed — all systems restored to normal operation";
  return (
    <div>
...
function TeamSplitFirst({
  ...
  const teamField = group.fields.find((f) => f.type === "team-table") as
    | Extract<ReportField, { type: "team-table" }>
    | undefined;
    ...
    <div>
      ...
      >
        ...
        >
          ...
          <span
            style={{ fontSize: 9.5, color: C.textMuted, fontStyle: "italic" }}
          >
            Continued on next page
          </span>
        </div>
        {/* First N members — border-top between rows; header border-bottom separates header from first row */}
        {teamField?.members.slice(0, firstN).map((m, idx) => (
          <div
            key={m.id}
            style={
              idx > 0 ? { borderTop: `0.5px solid ${C.cardBorder}` } : undefined
        ...
        >
          <span
            style={{ fontSize: 10, color: C.textMuted, fontStyle: "italic" }}
          >
            ··· {remaining} more team member{remaining !== 1 ? "s" : ""} listed
            on next page
          </span>
        </div>
      </div>
...
function TeamSplitCont({
  ...
  return (
    <div>
      ...
      >
        {teamField?.members.slice(fromN).map((m, idx) => (
          <div
            key={m.id}
            style={
              idx > 0 ? { borderTop: `0.5px solid ${C.cardBorder}` } : undefined
...
// Ordered-list section — first N items on this page, rest continue on next
function TimelineSplitFirst({
  group,
  splitFieldId,
  firstN,
  totalN,
  ...
  const fieldIdx = group.fields.findIndex((f) => f.id === splitFieldId);
  const tlField = group.fields[fieldIdx] as
    | Extract<ReportField, { type: "ordered-list" }>
    | undefined;
  const fieldsBefore = group.fields.slice(0, fieldIdx);
  const remaining = totalN - firstN;
  return (
    <div>
      <SecTitle title={group.title} />
      {fieldsBefore.length > 0 && (
        <div style={{ marginBottom: FIELD_GAP }}>
          <GenericFieldList fields={fieldsBefore} />
        </div>
      )}
      <div
        style={{ display: "flex", flexDirection: "column", gap: TL_ITEM_GAP }}
      >
        {tlField?.items.slice(0, firstN).map((it) => (
      ...
      >
        ···&nbsp;{remaining} more item{remaining !== 1 ? "s" : ""} continued on
        next page
      </div>
    </div>
  );
...
// Ordered-list section — continuation showing items from `fromN` onward
function TimelineSplitCont({
  group,
  splitFieldId,
  fromN,
}: {
  ...
  const tlField = group.fields[fieldIdx] as
    | Extract<ReportField, { type: "ordered-list" }>
    | undefined;
  const fieldsAfter = group.fields.slice(fieldIdx + 1);
  return (
    <div>
      <SecTitle title={group.title} continued />
      <div
        style={{ display: "flex", flexDirection: "column", gap: TL_ITEM_GAP }}
      >
        {tlField?.items.slice(fromN).map((it) => (
          <TimelineRow key={it.id} item={it} />
        ))}
      </div>
      {fieldsAfter.length > 0 && (
        <div style={{ marginTop: FIELD_GAP }}>
          <GenericFieldList fields={fieldsAfter} />
        </div>
      )}
    </div>
  );
...
function renderContentItem(item: ContentItem): ReactNode {
  if (item.kind === "desc") {
    return (
      ...
      >
        ...
        <div style={{ fontSize: 12.5, color: C.textDark, lineHeight: 1.6 }}>
          {item.text}
        </div>
      </div>
    );
  ...
  const g = item.group;
  if (g.predefinedId === "INCIDENT_CLOSURE")
    return <IncidentClosureSection group={g} />;
  if (g.predefinedId === "LESSONS_LEARNED")
    return <LessonsLearnedSection group={g} />;
...
function renderPageItem(pi: PageItem): ReactNode {
  ...
  if (pi.type === "team-first")
    return (
      <TeamSplitFirst group={pi.group} firstN={pi.firstN} totalN={pi.totalN} />
    );
  if (pi.type === "team-cont")
    return <TeamSplitCont group={pi.group} fromN={pi.fromN} />;
  if (pi.type === "tl-first")
    return (
      <TimelineSplitFirst
        group={pi.group}
        splitFieldId={pi.splitFieldId}
        firstN={pi.firstN}
        totalN={pi.totalN}
      />
    );
...
function pageItemKey(pi: PageItem): string {
  if (pi.type === "whole") return pi.item.key;
  if (pi.type === "team-first") return `tf:${pi.group.id}`;
  if (pi.type === "team-cont") return `tc:${pi.group.id}:${pi.fromN}`;
  if (pi.type === "tl-first") return `tlf:${pi.group.id}:${pi.splitFieldId}`;
  if (pi.type === "tl-cont")
    return `tlc:${pi.group.id}:${pi.splitFieldId}:${pi.fromN}`;
  return "unknown";
}
...
function computePages(
  ...
  memberH: Map<string, number[]>, // itemKey → per-member heights
  tlItemH: Map<string, number[]>, // "itemKey:fieldId" → per-item heights
): PageItem[][] {
  ...
  function avail() {
    return result.length === 0 ? PAGE1_AVAIL : PAGE_CONT_AVAIL;
  }
  function flush() {
  ...
  function push(pi: PageItem, h: number) {
    cur.push(pi);
    usedH += h;
  }
  ...
  for (let qi = 0; qi < queue.length; qi++) {
    ...
    if (entry.q === "content") {
      ...
      if (cur.length > 0 && usedH + h + ITEM_GAP > avail()) {
        if (item.kind === "group") {
          // ── Try team-table split ──────────────────────────────────────────
          const tf = item.group.fields.find((f) => f.type === "team-table") as
            | Extract<ReportField, { type: "team-table" }>
            | undefined;
          const mH = tf ? (memberH.get(item.key) ?? []) : [];
          if (tf && mH.length > 1) {
            const availForMembers =
              avail() - usedH - TEAM_FIRST_OVERHEAD - ITEM_GAP;
          ...
          // ── Try ordered-list split ────────────────────────────────────────
          const tlField = item.group.fields.find(
            (f) => f.type === "ordered-list",
          ) as Extract<ReportField, { type: "ordered-list" }> | undefined;
          if (tlField && tlField.items.length > 1) {
            const tlKey = `${item.key}:${tlField.id}`;
            const iH = tlItemH.get(tlKey) ?? [];
            if (iH.length > 1) {
              const n = iH.length;
              const sumItems = iH.reduce((a, b) => a + b, 0);
              const listTotalH = sumItems + Math.max(0, n - 1) * TL_ITEM_GAP;
              const listIdx = item.group.fields.indexOf(tlField);
              const nonListH =
                (sectionH.get(item.key) ?? h) -
                listTotalH -
                (listIdx > 0 ? FIELD_GAP : 0);
              // Only try if the non-list header portion actually fits on the current page
              const availForItems =
                avail() - usedH - nonListH - TL_SPLIT_HINT_H - ITEM_GAP;
              if (
                nonListH > 0 &&
                nonListH <= avail() - usedH &&
                availForItems > 0
              ) {
                let fit = 0,
                  cumH = 0;
                for (let i = 0; i < n; i++) {
                  const gap = fit > 0 ? TL_ITEM_GAP : 0;
                  if (cumH + iH[i] + gap > availForItems) break;
                  cumH += iH[i] + gap;
                  fit++;
                }
                if (fit >= 1 && fit < n) {
        ...
        // No split possible — flush and place whole on next page
        flush();
        push({ type: "whole", item }, h + ITEM_GAP);
      } else {
        push({ type: "whole", item }, h + ITEM_GAP);
      }
    } else if (entry.q === "team-cont") {
      ...
      if (cur.length > 0 && usedH + h + ITEM_GAP > avail()) {
        ...
        if (availForMembers > 0 && totalN - fromN > 1) {
          ...
          if (fit >= 1 && fit < totalN - fromN) {
            push(
              { type: "team-first", group, firstN: fromN + fit, totalN },
              avail() - usedH,
            );
            flush();
            queue.splice(qi + 1, 0, {
      ...
      // tl-cont: continuation of a split ordered-list section
      const { group, splitFieldId, fromN, totalN, itemHeights } = entry;
      const remItems = itemHeights.slice(fromN);
      const remH =
        remItems.reduce((a, b) => a + b, 0) +
        Math.max(0, remItems.length - 1) * TL_ITEM_GAP;
      const h = SEC_TITLE_EST + remH;
      if (cur.length > 0 && usedH + h + ITEM_GAP > avail()) {
        // Try sub-split of the continuation
        const availForItems =
          avail() - usedH - SEC_TITLE_EST - TL_SPLIT_HINT_H - ITEM_GAP;
        if (availForItems > 0 && totalN - fromN > 1) {
          let fit = 0,
            cumH = 0;
          for (let i = fromN; i < itemHeights.length; i++) {
            const gap = fit > 0 ? TL_ITEM_GAP : 0;
            if (cumH + itemHeights[i] + gap > availForItems) break;
            cumH += itemHeights[i] + gap;
            fit++;
          }
          if (fit >= 1 && fit < totalN - fromN) {
            push(
        ...
        flush();
        push({ type: "tl-cont", group, splitFieldId, fromN }, h + ITEM_GAP);
      } else {
        push({ type: "tl-cont", group, splitFieldId, fromN }, h + ITEM_GAP);
      }
    }
    ...
    >
      ...
      {severityField?.value ? (
        <span
          style={{
            ...
            letterSpacing: "0.06em",
            textTransform: "uppercase" as const,
            background: severityColor(severityField.value),
          }}
        >
          {severityField.value}
        </span>
      ...
      {dateField?.value && (
        <div style={{ marginLeft: "auto", textAlign: "right" }}>
          ...
          <strong style={{ color: "#fff", fontSize: 12, fontWeight: 500 }}>
            {formatDateTimeDisplay(dateField.value)}
          </strong>
        </div>
      )}
    </div>
...
function PageFooter({
  ...
  s,
  metadata,
}: MetaProps & { pageNum: number; totalPages: number; isLast: boolean }) {
  return (
    <div
      style={{
        background: C.cardBg,
    ...
    >
      <div style={{ fontSize: 10.5, color: C.textMuted }}>
        ...
        {!isLast && (
          <>
            &nbsp;·&nbsp;
            <span style={{ color: C.green, fontWeight: 500 }}>Continued →</span>
          </>
        )}
      </div>
      {isLast && (
...
export function ReportPreview({ data, settings, pdfMode = false }: Props) {
  ...
  const detailsGroup = groups.find(
    (g) => g.predefinedId === "INCIDENT_DETAILS",
  );
  const bodyGroups = groups.filter(
    (g) => g.predefinedId !== "INCIDENT_DETAILS",
  );

  const severityField = detailsGroup?.fields.find(
    (f) => f.type === "select" && f.label.toLowerCase().includes("severity"),
  ) as Extract<ReportField, { type: "select" }> | undefined;
  const dateField = detailsGroup?.fields.find((f) => f.type === "date-time") as
    | Extract<ReportField, { type: "date-time" }>
    | undefined;
  const descField = detailsGroup?.fields.find(
    (f) => f.type === "text-input",
  ) as Extract<ReportField, { type: "text-input" }> | undefined;
  const extraFields =
    detailsGroup?.fields.filter(
      (f) => f !== severityField && f !== dateField && f !== descField,
    ) ?? [];
  ...
  if (descField?.value)
    items.push({ key: "__desc__", kind: "desc", text: descField.value });
  if (extraFields.length)
    items.push({ key: "__extra__", kind: "extra", fields: extraFields });
  bodyGroups.forEach((g) => items.push({ key: g.id, kind: "group", group: g }));

  // Pagination state
  const [pages, setPages] = useState<PageItem[][]>(() => [
    items.map((item) => ({ type: "whole" as const, item })),
  ]);
  ...
  useLayoutEffect(() => {
    ...
    // Timeline item heights (data-tl-item: "itemKey:fieldId:itemId") → keyed by "itemKey:fieldId"
    const tlItemH = new Map<string, number[]>();
    el.querySelectorAll<HTMLElement>("[data-tl-item]").forEach((node) => {
      const parts = node.dataset.tlItem!.split(":");
      const key = `${parts[0]}:${parts[1]}`;
      if (!tlItemH.has(key)) tlItemH.set(key, []);
      tlItemH.get(key)!.push(node.getBoundingClientRect().height);
    });

    const newPages = computePages(items, sectionH, rawMemberH, tlItemH);
    const newKey = newPages.map((p) => p.map(pageItemKey).join(",")).join("|");
    ...
    <>
      ...
      */}
      ...
      >
        ...
        {/* Per-member heights for team-table split */}
        {items.flatMap((item) => {
          ...
          return tf.members.map((m) => (
            <div
              key={`${item.key}:${m.id}`}
              data-member={`${item.key}:${m.id}`}
            >
              <TeamMemberRow member={m} />
            </div>
          ));
        })}
        {/* Per-item heights for ordered-list split */}
        {items.flatMap((item) => {
          if (item.kind !== "group") return [];
          return item.group.fields.flatMap((f) => {
            if (f.type !== "ordered-list") return [];
            return f.items.map((tlItem) => (
              <div
                key={`${item.key}:${f.id}:${tlItem.id}`}
                data-tl-item={`${item.key}:${f.id}:${tlItem.id}`}
              >
                <TimelineRow item={tlItem} />
              </div>
            ));
          });
        })}
      ...
      {/* Paginated A4 pages */}
      <div data-layout-complete={layoutComplete ? "true" : undefined}>
        {pages.map((pageItems, pageIdx) => (
          <div
            key={pageIdx}
            style={
              pdfMode && pageIdx < pages.length - 1
              ...
              >
                ...
                {pageItems.map((pi, idx) => (
                  <div
                    key={pageItemKey(pi)}
                    style={{
                      marginBottom: idx < pageItems.length - 1 ? ITEM_GAP : 0,
                    }}
                  >
                    {renderPageItem(pi)}
                  </div>
